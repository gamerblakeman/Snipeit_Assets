from SnipeAsset.Update import createModel, pullModel, updateModel
import json
import html
...
for i in data["rows"]:
    ...
    outtable[id] = {'id': id, 'number': number, 'name': name, 'catNo': catNo}
...
matchtable = {}
for i in outtable:
    if("Unknown" in outtable[i]['name']):
        print(f"Skipping unknown item with id {i}")
        continue
    found = 0
    total += 1
    for x in DiviceTypes:
        if(outtable[i]['name'] == DiviceTypes[x]):
            print(f"Match found for {outtable[i]['name']} with id {i} and ID {x}")
            matchtable[i] = x
            matches += 1
            found = 1
            break
    if(found == 0):
        print(f"No match found for {outtable[i]['name']} with id {i}")
        failed.append(outtable[i]['name'])
print(f"{matches} matches found out of {total} total items")
print(f"Failed items: {failed}")

for i in matchtable:
    print(f"ID {i} matched with ID {matchtable[i]}")
    print(updateModel(URI, Key, i, matchtable[i], outtable[i]['name'], outtable[i]['catNo']))
