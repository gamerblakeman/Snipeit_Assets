...
class SnipeITAsset:
    ...
    def importfromFluke(self, infile):
        ...
        for d in self.dataout:
            ...
            try:
                self.divice['itemtype'] = self.DiviceTypes[int(d['des1'])]
                        ...
                        try:
                            ...
                            if(data[index] in learned):
                                ...
                                try:
                                    ...
                                    din = 0
                        self.divice["Type_ID"] = din
                        self.divice["itemtype"] = self.DiviceTypes[int(din)]
                    except KeyError:
    ...
    def exporttoCSV(self, outfile="Output.csv"):
        ...
        for a in self.Appliances:
            ...
            if("/" in self.Appliances[a]['date']):
                nextdate = (datetime.datetime.strptime(self.Appliances[a]['date'], "%d-%b-%y") + datetime.timedelta(days=365)).strftime("%Y-%d-%m")
            else:
                nextdate = str(int(self.Appliances[a]['date'].split('-')[0]) + 1) + "-" + self.Appliances[a]['date'].split('-')[1] + "-" + self.Appliances[a]['date'].split('-')[2]
            output += f"{a},{a},{self.Appliances[a]['itemtype']},{nextdate},{self.Appliances[a]['OverallResult']},{self.Appliances[a]['location']}\n"
